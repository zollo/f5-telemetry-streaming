 ...
 */
function getAccessToken(serviceAccount, httpAgent) {
    ...
    if (typeof tokenCache.promiseCache[tokenId] === 'undefined') {
        let httpOptions = {};
        if (serviceAccount.useServiceAccountToken) {
            const scopes = querystring.stringify({ scopes: TOKEN_SCOPES.join(',') });
            httpOptions = {
                headers: {
                    'Metadata-Flavor': 'Google'
                },
                method: 'GET',
                fullURI: `${METADATA_URL}/v1/instance/service-accounts/${serviceAccount.serviceEmail}/token?${scopes}`
            };
        } else {
        ...
        tokenCache.promiseCache[tokenId] = requestsUtil.makeRequest(httpOptions)
            .then((token) => {
                delete tokenCache.promiseCache[tokenId];
                return tokenCache.cacheToken(tokenId, token).access_token;
            },
            (err) => {
                delete tokenCache.promiseCache[tokenId];
    ...
    return tokenCache.promiseCache[tokenId];
}

...
describe('REST API / "/declare" endpoint', () => {
    ...
    beforeEach(async () => {
        ...
        await processDeclaration(testUtil.deepCopy(declaration));
    });

    afterEach(async () => {
        await processDeclaration(testUtil.deepCopy(dummies.declaration.base.decrypted()));
    ...
    [
        ...
        it('should response with 500 when caught error on attempt to POST declaration', async () => {
            ...
            const restOp = await sendRequest({
                path: `${uriPrefix}${dlURI}`,
                method: 'POST',
                body: testUtil.deepCopy(declaration)
            });
        ...
        it('should return 503 on attempt to POST declaration while previous one is still in process', async () => {
            ...
            const postDeclaration = () => {
                const copy = testUtil.deepCopy(declaration);
            ...
            const results = await Promise.all([
                postDeclaration(),
                testUtil.sleep(10).then(postDeclaration),
                testUtil.sleep(10).then(postDeclaration),
                testUtil.sleep(10).then(postDeclaration)
            ]);
    ...
    it('should return 503 on attempt to POST declaration while previous one is still in process (mixed)', async () => {
        ...
        let results = await Promise.all([
            postDeclaration(),
            testUtil.sleep(10).then(postDeclaration),
            testUtil.sleep(10).then(() => postDeclaration('namespace1')),
            testUtil.sleep(10).then(() => postDeclaration('namespace2'))
        ]);
        ...
        results = await Promise.all([
            ...
            testUtil.sleep(10).then(postDeclaration),
            testUtil.sleep(10).then(() => postDeclaration('namespace3')),
            testUtil.sleep(10).then(postDeclaration)
        ]);

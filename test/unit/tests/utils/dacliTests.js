...
const bigipConnTests = require('@f5-telemetry-tests/unit-shared/tests/bigipConn');
const bigipCredsTest = require('@f5-telemetry-tests/unit-shared/tests/bigipCreds');
const BigIpApiMock = require('@f5-telemetry-tests/unit-shared/bigipAPIMock');
const rootDir = require('@f5-telemetry-tests/common/rootdir');
const sinon = require('sinon');
const testUtil = require('@f5-telemetry-tests/unit-shared/util');

const dacli = rootDir.appImport('lib/utils/dacli');
...
describe('Device Async CLI', () => {
    ...
    const combinations = testUtil.product(
        ...
        .filter(([hostConf, credentialsConf]) => !(
            hostConf.value === remotehost && credentialsConf.value && !credentialsConf.value.passphrase
        ));

    combinations.forEach(([hostConf, credentialsConf, connectionConf], idx) => {
        describe(`(${idx + 1} / ${combinations.length}) host = ${hostConf.name}, user = ${credentialsConf.name}, connection = ${connectionConf.name}`, () => {
            ...
            const scriptCombinations = testUtil.product(
                ...
                .filter(([scriptConf, outputConf, partitionConf, folderConf]) => (
                    ((scriptConf && outputConf && partitionConf && folderConf))
                    && !(typeof partitionConf.value === 'undefined' && typeof folderConf.value !== 'undefined')
                ));

            scriptCombinations.forEach(([scriptConf, outputConf, partitionConf, folderConf], cidx) => {
                describe(`(${cidx + 1} / ${scriptCombinations.length}) script config: script = ${scriptConf.name} output = ${outputConf.name} partition = ${partitionConf.name} folder = ${folderConf.name}`, () => {
            ...
            testUtil.product(
                ...
                it(`should fail on error "${errorType}" ("${stepName}" step)`, async () => {
                    ...
                    execSteps.forEach((sname, eidx) => {
                        let expectedCallCount = execSteps.indexOf(stepName) >= eidx ? 1 : 0;
                        if (

...
const API = rootDir.appImport('lib/consumers/api');
...
class SplunkModule extends API.ConsumerModule {
    ...
    createConsumer() {
        assert.ok(this.isActive);
        return this.consumerInstances[this.consumerInstances.push(new SplunkConsumer()) - 1];
    }
...
module.exports = {
    load() {
        return MODULE_INSTANCES[MODULE_INSTANCES.push(new SplunkModule()) - 1];
    },
    getInstances() {

...
describe('Declarations -> Examples', () => {
    ...
    describe('Validate Example Declarations from examples/declarations', () => {
        testUtil.fs.listFiles(rootDir.resolve('examples/declarations'))
    ...
    describe('Validate Example Declarations from test/packages/func-shared/declarations', () => {
        testUtil.fs.listFiles(rootDir.resolve('test/packages/func-shared/data/declarations'))
            .filter((fpath) => fpath.endsWith('.json'))
            .forEach((fpath) => {
                it(`should validate example: ${fpath}`, () => {
                    const data = JSON.parse(fs.readFileSync(fpath));

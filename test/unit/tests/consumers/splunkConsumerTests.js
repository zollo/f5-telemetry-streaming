...
const splunkIndex = rootDir.appImport('lib/consumers/Splunk/index');
            ...
            it(`should ignore events from Event Listener in ${format} format`, () => {
                ...
                return splunkIndex(context)
                    .then(() => {
                        ...
                        assert.notStrictEqual(context.logger.verbose.callCount, 0, 'should have verbose messages');
                        assert.ok(/No data to forward/.test(context.logger.verbose.lastCall.args[0]));
                    });
            });

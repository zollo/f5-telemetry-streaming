...
describe('iHealth / Poller ', () => {
    ...
    describe('configuration variations', () => {
        const combinations = testUtil.product(
            ...
            .filter(([demoConf, intervalConf, systemAuthConf, systemConf]) => (
                !(systemConf.value && systemConf.value.host === remotehost
                && !(systemAuthConf.value && systemAuthConf.value.passphrase))
            ));

        combinations.forEach(([demoConf, intervalConf, systemAuthConf, systemConf], cidx) => describe(
            `(${cidx + 1} / ${combinations.length}) type = ${demoConf.name}, interval = ${intervalConf.name}, system = ${systemConf.name}, systemAuth = ${systemAuthConf.name}`,
            () => {

...
const assertLib = rootDir.appImport('lib/utils/assert');
const DataPipeline = rootDir.appImport('lib/dataPipeline');
const IHealthService = rootDir.appImport('lib/ihealth');
const IHealthPoller = rootDir.appImport('lib/ihealth/poller');
...
describe('iHealth / iHealth DEMO Service', () => {
    ...
    describe('configuration variations', () => {
        const combinations = testUtil.product(
            ...
            .filter(([intervalConf, proxyConf, systemAuthConf, systemConf]) => !(
                systemConf.value && systemConf.value.host === remotehost
                    && !(systemAuthConf.value && systemAuthConf.value.passphrase)
            ));

        // eslint-disable-next-line max-len
        combinations.forEach(([intervalConf, proxyConf, systemAuthConf, systemConf, namespaceConf, enableConf], cidx) => describe(
            `(${cidx} / ${combinations.length}) interval = ${intervalConf.name}, proxy = ${proxyConf.name}, system = ${systemConf.name}, systemAuth = ${systemAuthConf.name}, namespace = ${namespaceConf.name}, enable = ${enableConf.name}`,
            () => {

...
const dummies = require('@f5-telemetry-tests/unit-shared/dummies');
...
const DataPipeline = rootDir.appImport('lib/dataPipeline');
const ResourceMonitor = rootDir.appImport('lib/resourceMonitor');
const SafeEventEmitter = rootDir.appImport('lib/utils/eventEmitter');
const SystemPollerService = rootDir.appImport('lib/systemPoller');
const SystemPoller = rootDir.appImport('lib/systemPoller/poller');
...
describe('System Poller / System Poller Service', () => {
    ...
    describe('configuration variations', () => {
        const combinations = testUtil.product(
            ...
            .filter(([endpointsConf, systemAuthConf, systemConf]) => !(
                systemConf.value && systemConf.value.host === remotehost
                    && !(systemAuthConf.value && systemAuthConf.value.passphrase)
            ));

        combinations.forEach(([endpointsConf, systemAuthConf, systemConf, namespaceConf], idx) => describe(
            `(${idx + 1} / ${combinations.length}) endpoints = ${endpointsConf.name}, system = ${systemConf.name}, systemAuth = ${systemAuthConf.name}, namespace = ${namespaceConf.name}`,
            () => {

...
describe('System Poller / Poller ', () => {
    ...
    describe('configuration variations', () => {
        const combinations = testUtil.product(
            ...
            .filter(([pollConf, endpointsConf, systemAuthConf, systemConf]) => !(
                systemConf.value && systemConf.value.host === remotehost
                    && !(systemAuthConf.value && systemAuthConf.value.passphrase)
            ));

        combinations.forEach(([pollConf, endpointsConf, systemAuthConf, systemConf], idx) => describe(
            `(${idx + 1} / ${combinations.length}) endpoints = ${endpointsConf.name}, system = ${systemConf.name}, systemAuth = ${systemAuthConf.name}`,
            () => {
                ...
                if (pollConf.value) {
                    ...
                    it('should keep only 40 records in the history', async () => {
                        ...
                        info = poller.info();
                        const idx2 = info.state.history.findIndex((rec) => {
                            try {
                        ...
                        assert.isNumber(idx2);
                        assert.isAbove(idx2, 0);
                        assert.isBelow(idx2, info.state.history.length - 1);

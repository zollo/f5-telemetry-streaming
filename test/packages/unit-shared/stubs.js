...
const pkgConfig = require('./config');
const waitTill = require('./util').waitTill;

const tsLogsForwarder = pkgConfig.getDefaultLogger().tsLogger;
...
const DEFAULT_STUB_SRC_MAP = Object.freeze({
    ...
    configWorker: 'lib/config',
    deviceUtil: 'lib/utils/device',
    logger: 'lib/logger',
    resourceMonitorUtils: 'lib/resourceMonitor/utils',
    storage: 'lib/storage',
    tracer: 'lib/utils/tracer',
    utilMisc: 'lib/utils/misc'
});
...
const CONSUMERS_PATHS_LIST = [
    'lib/consumers',
    'tests/consumers',
    'tests/dataPipeline/consumers'
];
...
function loadModules(modules, srcMap) {
    ...
    Object.entries(loaded)
        .forEach(([name, path]) => {
            loaded[name] = rootDir.appImport(path);
        });
...
const _module = module.exports = {
    ...
    coreStub(coreModules, options) {
        ...
        if (coreModules.deviceUtil || coreModules.tracer || coreModules.utilMisc) {
            coreModules.utilMisc = coreModules.utilMisc || rootDir.appImport('lib/utils/misc');
    ...
    utilMisc(utilMisc) {
        ...
        ['access', 'readdir', 'stat'].forEach((method) => {
            ...
            ctx.fs.fs[method] = function customFN(fsPath) {
                if (CONSUMERS_PATHS_LIST.some((p) => fsPath.includes(p))) {

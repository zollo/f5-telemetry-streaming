        ...
        function generateTestsForExporter(exporter, last) {
            ...
            describe('Configure TS and generate data', () => {
                ...
                testUtils.shouldSendListenerEvents(
                    harness.bigip,
                    (bigip, proto, port, idx) => `functionalTestMetric="147",EOCTimestamp="1231232",hostname="${bigip.hostname}",testDataTimestamp="${testDataTimestamp}",test="true",testType="${OTEL_COLLECTOR_CONSUMER_NAME}",protocol="${proto}",msgID="${idx}",exporter="${exporter}"\n`
                );
            });
            ...
            describe('System Log data', () => {
                harness.bigip.forEach((bigip) => it(
                    ...
                    () => bigip.ssh.default.exec('cat /var/log/restnoded/restnoded.log', (err, streamData) => {
                        if (err) {

...
# DEV RPM: f5-telemetry-1.41.0-0.20240107071243.28507f40.dev_build_info.noarch.rpm
# Release RPM: f5-telemetry-1.41.0-0.noarch.rpm

PKG_PATH="./application/package.json"
...
package_name () {(
    node -e "console.log(require('${PKG_PATH}').name);"
)}

package_version () {(
    node -e "v=require('${PKG_PATH}').version.split('-');v.length===1?v.push('1'):'';console.log(v.join('-'));"
)}
